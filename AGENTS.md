...
- The main menu uses native `#duel-settings` and `#menu-utilities` disclosures closed by default. `#match-configuration-summary` is derived only from the four `selected*` values and must refresh after selection, language, and Arcade restoration; Help/Controls return focus inside `#menu-utilities` without collapsing it.
- Contextual CPU tactics remain rule-based: one observed attack sequence can trigger one whiff opportunity, crouch only answers dominant punch patterns, bait reuses retreat away from walls, air attacks use real hitboxes once per jump, and timed late-round pressure only suppresses retreat when the CPU is behind; sustained blocking uses accumulated memory rather than held-input reads.
