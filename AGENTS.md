...
- Manual smoke test: menu appears, `Tab` shows visible focus, difficulty selector changes CPU behavior, arena selector changes background, `Reducir movimiento` persists and reduces impact shake, `AYUDA` opens help, `VOLVER` returns, `INICIAR JUEGO` starts, `A/D/W/S/J/K/L` work, arrow keys move/jump/crouch, `C` crouches, `S` still blocks, combos `J,J`, `J,K`, and `K,K` work and show distinct visual feedback, `P`/`Esc` pause and resume with round/score/time/difficulty/arena summary, timer counts down during play, health bars animate after damage, stats update after game over, portrait phone shows orientation warning, canvas stays proportional after resize, game over appears at `0%`, `REINICIAR` restarts, `MENU` returns to menu.
